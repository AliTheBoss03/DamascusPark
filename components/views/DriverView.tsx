...
const DRIVER = MOCK_USERS[0]; // Ahmad Al-Masri
...
export function DriverView() {
  ...
  // Simulate occasional offline state for demo
  useEffect(() => {
    const id = setInterval(() => {
      // Rarely flip online status to show resilience
    }, 30000);
  ...
  const handleStartSession = () => {
    ...
    if (balance < hourlyRate) {
      showNotif("error", "Insufficient wallet balance for this zone.");
      return;
    }
    ...
    showNotif("success", `Parking started in ${selectedZone.name}`);
  };
  ...
  const handleEndSession = () => {
    ...
    showNotif(
      "info",
      `Session ended. Charged ${formatCredits(charged)} credits.`
    );
  };
  ...
  const handleRedeem = async (
    ...
    const card = scratchCards.find(
      (c) => c.pin.toUpperCase() === pin && !c.is_used
    );
    if (!card) {
      const used = scratchCards.find((c) => c.pin.toUpperCase() === pin);
      return { error: used ? "This voucher has already been used." : "Invalid PIN. Please check and try again." };
    }
    setScratchCards((prev) =>
      prev.map((c) => (c.id === card.id ? { ...c, is_used: true } : c))
    );
  ...
  const currentCost = activeSession
    ? calcSessionCost(selectedZone, GAS_PRICE, activeSession.started_at)
    : 0;
  const hourlyRate = calcHourlyRate(selectedZone, GAS_PRICE);
  ...
  return (
    <div className="space-y-4 animate-fade-in">
      ...
      <div className="flex items-center justify-between">
        <h2 className="text-sm font-semibold text-slate-400">My Dashboard</h2>
        <div
      ...
      {activeSession ? (
            ...
            <div className="flex items-start justify-between mb-4">
              <div>
                ...
                <h3 className="text-lg font-bold text-slate-100 font-mono">
                  {activeSession.license_plate}
                </h3>
              </div>
              <ZoneBadge
                color={selectedZone.zone_color}
                name={selectedZone.name}
                nameAr={selectedZone.name_ar}
                size="sm"
              />
            </div>
            ...
            <div className="grid grid-cols-3 gap-3 mb-4">
              ...
              <div className="card-elevated p-3 text-center">
                ...
                <p className="text-sm font-bold text-amber-400 tabular-nums">
                  {formatCredits(currentCost)} cr
                </p>
              </div>
              <div className="card-elevated p-3 text-center">
                <p className="text-xs text-slate-500 mb-1">Rate/hr</p>
                <p className="text-sm font-semibold text-slate-300 tabular-nums">
                  {formatCredits(hourlyRate)} cr
                </p>
              </div>
            </div>
        ...
        <div className="card p-5 space-y-4">
          ...
          <div>
            ...
            <div className="grid grid-cols-3 gap-2">
              {MOCK_ZONES.map((zone) => (
                ...
                >
                  <span
                    className={`w-2.5 h-2.5 rounded-full block mb-2 ${getZoneDotColor(zone.zone_color)}`}
                  />
                  <p className="text-xs font-semibold text-slate-200 leading-tight">
                    {zone.name.split("/")[0].trim()}
                  </p>
                  <p className="text-xs text-slate-500 mt-0.5">
                    {formatCredits(calcHourlyRate(zone, GAS_PRICE))}/hr
                  </p>
                </button>
              ))}
            </div>
          ...
          <div className="flex items-center gap-2 text-xs text-slate-500 bg-slate-800/50 rounded-xl p-3">
            ...
            <span>
              GPS detected: <span className="text-slate-300">{selectedZone.name}</span>
              {" "}·{" "}
              <span className="text-amber-400">
                {formatCredits(hourlyRate)} credits/hr
              </span>
            </span>
          </div>
      ...
      {/* Map */}
      <DamascusMap
        activeZone={activeSession ? selectedZone : null}
        showDriverPin={true}
      />
